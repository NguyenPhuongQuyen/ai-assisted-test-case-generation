...
def upgrade() -> None:
    ...
    test_case_priority = sa.Enum("high", "medium", "low", name="test_case_priority")
    test_case_status = sa.Enum(
        "draft", "in_review", "needs_fix", "approved", "exported", "rejected", name="test_case_status"
    )
    ...
    op.create_table(
        ...
        sa.Column("parent_id", sa.Integer(), sa.ForeignKey("modules.id", ondelete="SET NULL"), nullable=True),
    ...
    op.create_table(
        "requirements",
        sa.Column("id", sa.Integer(), primary_key=True),
        sa.Column("module_id", sa.Integer(), sa.ForeignKey("modules.id", ondelete="RESTRICT"), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column("id", sa.Integer(), primary_key=True),
        sa.Column(
            "requirement_id", sa.Integer(), sa.ForeignKey("requirements.id", ondelete="RESTRICT"), nullable=False
        ),
        sa.Column("module_id", sa.Integer(), sa.ForeignKey("modules.id", ondelete="RESTRICT"), nullable=False),
